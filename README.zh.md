...
| 📈 **计分弹窗**   | 出牌瞬间在投影屏幕玩家头像旁显示该次出牌的实时得分。                     |
...
**`最终分 = 基础分 (人类2/AI 1) × 层1乘数 (手牌数) × 层2乘数 (颜色多样性) × 层3乘数 (同色连打)`**
...
主持人可在大厅界面底部通过「🤖 添加AI」按钮添加最多 **5 个 AI 玩家**。每个 AI 会获得一个随机名字并在末尾标注 `[AI]`，同时随机分配以下三种性格之一。**AI 玩家每次出牌贡献 +1/−1 分**（人类玩家为 +2/−2）。

| 性格 | 速度 | 出牌间隔 | 算法 | 行为特征 |
|---|---|---|---|---|
| **⚡ 速攻型** (_fast_) | 极快 | 最快 1 秒 | **简单** —— 直接打出手牌中第一张合法牌，无任何策略。 | 有 50% 概率忘记喊 UNO。病毒治愈率低（40%）。 |
| **⚖️ 均衡型** (_medium_) | 中等 | 最快 1.5 秒 | **战术** —— 优先打出同色的功能牌（跳过、反转、+2），然后同色牌，再同值牌，百搭牌留作最后手段。 | 可靠地喊 UNO。病毒治愈率较高（70%）。 |
| **🧠 策略型** (_slow_) | 较慢 | 最快 2.5 秒 | **智能** —— 优先打出同色 +2 进行攻击，再用跳过/反转冻结对手，然后打出手中持有最多的颜色（主导色）来控制局面，百搭 +4 和百搭牌留到最后使用。 | 必定喊 UNO。病毒治愈率极高（95%）。 |
